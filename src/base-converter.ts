...
export class BaseConverter {
  ...
  private validateNumberInBaseN(numberToConvert: string, baseFrom: number): void {
    ...
    const splittedNumberByDelimiter = numberToConvert.split(this.delimeter);
    ...
    const digits = Array.from(numberToConvert).filter((digit) => digit !== this.delimeter);
  ...
  convertFromBaseNToDecimal(numberToConvert: string, baseFrom: number): number {
    ...
    let digitArray = Array.from(numberToConvert).filter((digit) => digit !== this.delimeter);
    let position = numberToConvert.split('.')[0].length - 1;
    let result = 0;
  ...
  private convertDecimalPart(decimalPart: number, baseTo: number, precision: number) {
    ...
    for (let i = 0; i < precision; i++) {
      ...
      result += decimalPartCopy >= 10 ? this.numbersToLettersMap[integerPart] : integerPart;
      decimalPartCopy %= 1;
    }
    return result;
  }
   ...
   * @param _namedParameters An object with options for the convert method to work
